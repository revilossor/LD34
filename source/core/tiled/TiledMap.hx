 ...
 * 
 * Some modifications for haxeflixel 3.3.8 - moved classes out of flixel-addons so works with openfl-html5 
 * More modifications to make work with image layers
 * Oliver Ross
 * 
 */
class TiledMap
{
	...
	public var imageLayers:Array<TiledImageLayer>;
	...
	public function new(Data:Dynamic)
	{
		...
		if (source.has.backgroundcolor)
		{
			...
			if (bgColorStr != null) 
			{
				...
				backgroundColor = Std.parseInt(bgColorStr);
			}
		}
		...
		imageLayers = new Array<TiledImageLayer>();
		...
		for (node in source.nodes.imagelayer) {
			...
			if (!noLoadHash.exists(name)) {
				imageLayers.push(new TiledImageLayer(node, this));
			}
		}
